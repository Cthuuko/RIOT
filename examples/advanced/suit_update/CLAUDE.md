...
| **`PERFORMANCE.md`** | **Timing/throughput/runtime-memory instrumentation**: the `SUIT_PERF=1` opt-in build flag, the six measured operation classes (secret-value generation and signature verification are tier-sensitive; symmetric decryption, hashing, transport and storage are the tier-invariant control), the four memory indicators (worker-stack and heap high-water, static config dump, free RAM), the `PERFCFG`/`PERF`/`PERFMEM` CSV schemas, the per-tier capture procedure and the limits of the measurement. **§9 is device-side automation**: `device_perf_bench.py` runs N updates unattended on an already-flashed tier, reopening the console across each reboot (the thing the manual runbook could not do), triggering via `suit fetch` instead of `suit/notify`, and recovering `PERFCFG` every run; `sudo` is needed only to flash and to keep the CDC-ECM address alive (`suit-ecm-keeper.sh`), never inside the loop. **§8 is the host-side counterpart**: `SUIT_HOST_PERF=1`, the `HOSTCFG`/`HOSTPERF` schemas, the host↔device phase pairing table, and why the `host_crypto_bench.py` repeat harness — not a single publish — is the authoritative layer there. The counterpart to `CRYPTO_TIERS.md`, which measures space only |
| **`PERF_RESULTS_NRF52840.md`** | **Device results**: measured timing/runtime-memory of instrumented (`SUIT_PERF=1`) SUIT updates on real nRF52840 Dongle hardware, **medians over 47–50 repeats per crypto tier** (T1–T5, incl. the ES256 library control), captured with `device_perf_bench.py`. Headline: the full-PQC tier is the *fastest* (9.30 s vs classical 13.04 s); ML-DSA verification is 30–52× faster than Ed25519, decomposed into 4.9× library × 10.7× algorithm; ML-KEM decapsulation 18.5× faster than X25519, using 8,792 B of stack (**8,996 B worst case — only 220 B of margin**) and **zero heap**. What the repeats changed: the T4/T1/T5 stack peaks are higher than one run showed, `storage_write` is bimodal by slot (one 85 ms flash-page erase), all run-to-run variance is the CoAP transfer, and the T1-vs-T2 Ed25519 gap is real rather than noise |
| **`PERF_RESULTS_HOST.md`** | **Host results**: the *producer* half — signing, encapsulation, encryption on the build host (`SUIT_HOST_PERF=1` + the `host_crypto_bench.py` repeat harness), same five tiers so every number pairs with a device one. Headline: **the ranking inverts** — ML-DSA signing is the dearest signature on the host and the cheapest verification on the device; the T1→T3 trade is +269.9 µs host for −2,100 ms device (**7,782× leverage**); ML-DSA signing is a *distribution* (rejection sampling, ~20× spread) not a number; producer cryptography is ≤0.006 % of an update, so **no producer-side cost argument against PQC survives** |
| **`PERF_RUNBOOK_NRF52840.md`** | **Hardware capture runbook**: the exact per-tier command sequence for running instrumented (`SUIT_PERF=1`) updates on the nRF52840 Dongle across all four tiers — tier→key-directory table (no keygen needed, all keys already in-repo), one-time setup, the 8-step per-tier procedure, what to paste back, and how to read the results (payload size differs per tier, so the bulk phases compare as *throughput* only). **§4a is the automated path**: once a tier is flashed, `device_perf_bench.py --tier Tn --repeats 20` replaces steps 4-9 |
| **`CRYPTO_OPERATIONS.md`** | **Per-operation command + code-location reference**: signing/device keygen, the firmware build (which flags bind what into the image, §3), manifest generation, signing, manifest & payload encryption (host); manifest decryption, signature verification, streaming payload decryption (device). Per-algorithm tables for all 7 signature and 3 KEM schemes, measured artifact/overhead sizes, and real `suit-tool parse -j` JSON manifest examples (EdDSA + ML-DSA-65) |
...
- `dist/tools/suit/` — `gen_key.py`, `gen_manifest.py`, `password_protect_key.py`, `suit-manifest-generator/`,
  plus the host-side perf instrumentation: `hostperf.py` (opt-in via
  `SUIT_HOST_PERF=1`, no-op otherwise) and `host_crypto_bench.py` (the
  repeat harness). Consumers: `suit_tool/{create,sign}.py` and the three
  `examples/advanced/suit_update/*-encryption*/encrypt_*.py` tools.
  Device-side automation: `device_perf_bench.py` (N unattended OTA runs on a
  flashed tier), `perfstats.py` (summary/CSV format shared by both harnesses)
  and `suit-ecm-keeper.sh` (user-run sudo, keeps the CDC-ECM address across
  dongle reboots so the loop stays unprivileged)

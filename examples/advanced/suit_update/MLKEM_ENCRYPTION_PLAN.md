...
- [x] Step 5b (link-time part, 2026-07-19): samr21-xpro **feasibility matrix measured for all 12 combos** — see the table below. **⚠️ SUPERSEDED 2026-07-20, see MLKEM_ENCRYPTION_CHANGES.md's correction section: this measurement never accounted for wolfCrypt's runtime heap/stack use for ML-KEM (invisible to link-time `size`), which was wrong by ~3.3-3.6 KB. Corrected result: full-PQ (any ML-DSA level + ML-KEM) does NOT fit; Ed25519 + ML-KEM-768/1024 still fits, with less margin than claimed here.**
- [x] Step 5c (2026-07-20, hardware bring-up correction): real samr21-xpro hardware hit `CEK derivation failed: -125` (wolfCrypt `MEMORY_E`) on the full-PQ combo — traced to `wc_mlkem.c` heap-allocating scratch buffers (up to 6KB/call) regardless of static RAM headroom. Fixed via `WOLFSSL_NO_MALLOC` + `WOLFSSL_MLKEM_MAKEKEY_SMALL_MEM` + `WOLFSSL_MLKEM_ENCAPSULATE_SMALL_MEM` (moves buffers to a *measured* ~8.5-9KB peak stack, via `-fstack-usage`) and a corrected, enforced `SUIT_WORKER_STACKSIZE=9216` for any `ml-kem-%` build (previously 4096, silently insufficient - undetectable at link time, would have corrupted `.bss` at runtime on Cortex-M0+'s no-MPU). Re-measured with the fix: full-PQ ❌ overflows ~3.4-3.6KB (was falsely "1,160 B spare"); Ed25519+ML-KEM-768 ✅ 2.0-3.4KB spare depending on payload-encryption; Ed25519+ML-KEM-1024 ✅ but only 48B spare. See `FIRMWARE_ENCRYPTION_CHANGES.md` for the authoritative table.
- [x] Step 6: docs (MLKEM_ENCRYPTION_CHANGES.md correction section, FIRMWARE_ENCRYPTION_CHANGES.md authoritative table, SAMR21_EXAMPLES.md cookbook, CLAUDE.md) + refresh implementation patch (2026-07-20)

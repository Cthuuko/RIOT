...
- [x] Step 5b: E2E on samr21-xpro (2026-07-19) — **verified on real hardware** with Ed25519+X25519 encryption: full encrypted OTA over CoAP/ethos (577 B container → 485 B manifest → verify → flash slot 1 → reboot), mixed with a prior plain update on the same image (pass-through proven). Bring-up fix: E.7's publish path (`coaproot/fw/<app>/<board>/`, name must stay `riot.suit.enc` — notify URL is 61/64 chars). See `SAMR21_EXAMPLES.md` Example E (banner + E.9 status; tamper-on-hardware and repeat-cycles remain open there).

#!/usr/bin/env python3
"""Generate an ML-DSA-44 keypair, sign a message, and emit C headers.
...
demonstrates ML-DSA-44 (FIPS 204, NIST security category 2) signing as the
smallest ML-DSA parameter set — a post-quantum alternative to the Ed25519
...
def parse_arguments():
    ...
    parser.add_argument('--output', '-o', default="dsa44.pem",
...
def main(args):
    print("Trying out ML-DSA-44 - START")
    ...
    print(f"Public key: {len(pub_raw)} bytes")   # expect 1312
    print(f"Signature:  {len(signature)} bytes")  # expect 2420
    ...
    seed = pk_raw[:32]
    reloaded = MLDSA44PrivateKey.from_seed_bytes(seed)
    ...
    print("Trying out ML-DSA-44 - END")
